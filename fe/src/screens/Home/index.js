...
const HomeScreen = () => {
  ...
  const { data: trendingData, isLoading: isTrendingLoading } = useGetTrendingRecipesQuery();
  ...
  const { carouselRecipes, gridRecipes } = React.useMemo(() => {
    if (!recipes || recipes.length === 0) return { carouselRecipes: [], gridRecipes: [] };
    
    // Shuffle or just pick different ranges to avoid duplication
    const shuffled = [...recipes].sort(() => 0.5 - Math.random());
    return {
      carouselRecipes: shuffled.slice(0, 5),
      gridRecipes: recipes.filter(r => !shuffled.slice(0, 5).some(c => c.id === r.id)).slice(0, 10)
    };
  }, [recipes]);
  ...
  return (
    ...
    {/* Recommendation Carousel */}
                ...
                >
                  <View style={styles.recommendationContent}>
                    <View style={styles.recommendationHeader}>
                      <Text style={styles.recommendationTag}>{t('weekly_pick', 'Gợi ý cho bạn')}</Text>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>{t('featured_recipes')}</Text>
        ...
        {isTrendingLoading ? (
          <View style={{ marginVertical: 40, alignItems: 'center' }}>
            ...
            <Text style={{ marginTop: 10, color: Colors.textLight }}>{t('loading_recipes', 'Đang tìm kiếm món ngon...')}</Text>
          </View>
        ) : recipes.length === 0 ? (
          <View style={{ marginVertical: 60, alignItems: 'center', paddingHorizontal: 40 }}>
            <Ionicons name="restaurant-outline" size={60} color={Colors.primary} style={{ opacity: 0.5 }} />
            <Text style={{ marginTop: 20, fontSize: 18, fontWeight: '600', color: Colors.text, textAlign: 'center' }}>
              {t('preparing_recipes_title', 'Đang chuẩn bị thực đơn...')}
            </Text>
            <Text style={{ marginTop: 10, fontSize: 14, color: Colors.textLight, textAlign: 'center' }}>
              {t('preparing_recipes_desc', 'Chờ một chút nhé, chúng mình đang cập nhật những công thức nấu ăn mới nhất cho bạn.')}
            </Text>
